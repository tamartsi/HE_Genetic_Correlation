...
VCg2 <- rep(0,length(phenotypes));names(VCg2) <- phenotypes
...
for(i in 1:length(phenotypes)){ # i=1
  ...
  data = readLines(f)
  if(length(config$household_matrix_filename)>0){ #two matrix case
    ...
    mnG <- as.numeric(strsplit(rsG,"\t",fixed = T)[[1]][2])
    seG <- as.numeric(strsplit(rsG,"\t",fixed = T)[[1]][3])
    mnE <- as.numeric(strsplit(rsE,"\t",fixed = T)[[1]][2])
    seE <- as.numeric(strsplit(rsE,"\t",fixed = T)[[1]][3])
    nn <- as.numeric(strsplit(rsN,"\t",fixed = T)[[1]][2])
    
    VCg1[[pheno.1]] <- mnG/(mnG+mnE)
    VCe[[pheno.1]] <- mnE/(mnG+mnE)
...
if((length(config$phenotypes_fieldnames)==1) & (length(grep('-',config$phenotypes_fieldnames))==1)){   #10-100
  ...
  phenotypes_fieldnames <- colnames(phenotypes)[sst:sse]
}else{
  phenotypes_fieldnames <- config$phenotypes_fieldnames
}

phenotypes <- phenotypes[,c(config$idfield_name,covariates,phenotypes_fieldnames)]
...
phenotypes<-phenotypes_fieldnames
...
if(file.exists(tpname)){
  ...
  stop(paste0("Couldn't find accessory data: ",tpname))
}
...
for (i in 1:length(ttitle)){ # i=1
  ...
  ddata <- data.frame(
    name=factor(phenotypes_fieldnames,levels = rev(phenotypes_fieldnames)),
    value=rep(NA,length(phenotypes_fieldnames)),
    sd=rep(NA,length(phenotypes_fieldnames))
  )
  ...
  for( j in 1:length(phenotypes)){ #j=1
    ...
    if(length(config$household_matrix_filename)>0){ #two matrix case
      ...
      ddata[phenotypes[j],"value"] <- mnG
      ddata[phenotypes[j],"sd"] <- seG
    }    
  }  
